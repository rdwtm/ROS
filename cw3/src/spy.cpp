...
int main(int argc, char* argv[]) {
  ...
  ros::NodeHandle nh;
  ros::Subscriber sub1 = nh.subscribe<turtlesim::Pose>("turtle1/pose", 10, cel_cb);
  ros::Subscriber sub2 = nh.subscribe<turtlesim::Pose>("/turtle2/pose", 10, poz_cb);
  ...
  while (ros::ok()) {
    ...
    if (dist>0.5){
      ...
      wiadomosc.angular.z= 1*(angle-theta_zolw);
